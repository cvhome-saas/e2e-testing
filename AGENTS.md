...
Guidance for AI coding agents working in `e2e-testing`. It applies to the whole repository.

## Orientation

Playwright end-to-end suite for cvhome. Scaffold only so far (default config, one example spec, a Playwright CI workflow); no baseURL, no app journeys yet. The place browser regression tests go when they are written; k6 browser scripts in load-testing cover performance, not correctness.

Part of the `cvhome-saas` organisation. Cross-repo routing, review and releases live in
`cvhome-saas/orchestrator`; this file is the repo's own rulebook. Entry points: playwright.config.ts, tests/, .github/workflows/playwright.yml.
